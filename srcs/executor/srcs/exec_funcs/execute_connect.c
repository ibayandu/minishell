...
/*   Updated: 2025/08/04 12:49:30 by yzeybek          ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	execute_pipe_left(t_command *cmd, int pipefd[2], int *exit_code)
{
	int	status;
...
static void	execute_pipe_right(t_command *cmd, int pipefd[2], int *exit_code)
{
	int	status;
...
static int	execute_pipe(t_connect_cmd *connect, int *exit_code)
{
	...
	pid1 = fork();
	if (pid1 == 0)
		execute_pipe_left(connect->first, pipefd, exit_code);
	pid2 = fork();
	if (pid2 == 0)
		execute_pipe_right(connect->second, pipefd, exit_code);
	close(pipefd[0]);
	close(pipefd[1]);
	waitpid(pid1, NULL, 0);
