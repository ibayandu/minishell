...
/*   Updated: 2025/07/05 19:05:52 by yzeybek          ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_variable_tables(t_minishell *minishell)
{
	...
	if (!minishell->alias_variables)
		minishell->alias_variables = hash_create(VARIABLES_HASH_BUCKETS);
}
...
t_word	*param_expand(char *string, int *sindex, int *expanded_something, t_minishell *minishell)
{
	...
	else
	{
		...
		var = find_variable(temp1, minishell->global_variables);
		if (var && var->value)
		{
			temp = var->value;
			goto return0;
